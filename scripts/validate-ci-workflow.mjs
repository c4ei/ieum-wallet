...
const required = new Map([
  ...
  ["releaseAssetNamePattern: IEUM-Wallet-${{ matrix.label }}-v${{ needs.validate.outputs.display_version }}-[platform]-[arch][setup].[ext]", "번들 이름과 확장자를 중복한 릴리스 파일명을 사용하면 안 됩니다."],
  ["require(\"./version.json\").displayVersion", "릴리스 버전은 version.json 단일 기준에서 읽어야 합니다."],
  ["tags:\n      - \"v*.*.*.*\"", "4자리 운영 버전 태그가 릴리스 워크플로를 자동 실행해야 합니다."],
