...
- AES-256-GCM 암호문 그대로 `.aahvault` 금고 파일 내보내기
- 지갑 열기와 금고 가져오기 비밀번호 입력 상태 분리, 다운로드 파일명·저장 위치 안내
- 가져온 금고의 비밀번호·주소 무결성 검증과 기존 지갑 교체 경고
- 신규·SEED 복원 지갑의 무작위 3단어 백업 재확인
- 현재 비밀번호 확인 후 새 비밀번호로 금고 재암호화
- 연속 잠금 해제 실패 시 1초부터 최대 30초까지 단계적 지연
- 현재 비밀번호와 지갑 주소를 모두 확인하는 안전한 기기 초기화

## 1.0.2.2 - 2026-08-31

- 잠금 해제 후 사용할 수 있는 접이식 지갑 백업 화면 추가
- 새 지갑과 SEED 복원 지갑은 암호화 금고에 SEED를 보관하고 기존 지갑은 개인키 백업 제공
- USB 콜드월렛 영역을 기본 접힘 상태로 변경
- 완료·실패·유실 로컬 전송 내역 선택 삭제 제공, 처리 중 내역은 중복 송금 방지를 위해 보호

## 1.0.2.1 - 2026-08-27

- IEUM Manager의 확정 주소 거래를 조회해 받은 거래와 보낸 거래를 함께 표시
- `받음`, `보냄`, `내 주소 이동`을 구분하고 5건 단위 페이지 처리
- Manager 조회 실패를 잔액 조회·송금 실패와 분리
- Rust 프록시에서 공식 `https://iem.aah.name`만 허용해 임의 URL 요청 차단

## 1.0.1.1 - 2026-08-25

- 중복 클릭을 막는 전경 확인 모달과 제출 단계 표시
- 지연·유실 거래 판정 및 원래 서명 거래의 안전 재전파
- nonce·해시 재검증으로 이중 전송 차단
- `version.json` 단일 표시 버전과 자동 내부 버전 동기화
- 릴리스 사전 단계의 중복 웹 빌드 제거
